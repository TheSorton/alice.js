import { ButtonBuilder, ButtonStyle, ActionRowBuilder,
   EmbedBuilder, ComponentType, SlashCommandBuilder } from 'discord.js';
  ...
  .addStringOption(option =>
    ...
    async execute(interaction) {
      ...
      const buildEmbed = (data) => {
        ...
        const fields = [
          data.media[0].title.userPreferred && { name: 'Anime', 
            value: data.media[0].title.userPreferred, inline: false }
        ].filter(Boolean);
