...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let rankName;
    let ranking = data.rankings.find(
      (ranking) => ranking.type === "POPULAR" && ranking.season != null
    );
    if (!ranking) {
      ranking = data.rankings.find(
        (ranking) =>
    ...
    if (!data.title.english) {
      ...
      embed.setAuthor({
        name: `${data.title.english} (${data.title.native})`,
        url: data.siteUrl,
      });
    }
    if (data.description) {
