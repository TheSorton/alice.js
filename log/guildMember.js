	...
	if (guild) { 
		...
		if (msgDoc) {
			...
			const embed = new MessageEmbed()
				.newAuthor(`${member.displayName}`, member.user.avatarURL('png'))
				.setDescription(`Welcome ${member.displayName}! Read the rules, assign yourself a role, and enjoy your stay`)
				.setFooter(`You are the ${ordinal(member.guild.memberCount)} member to join`)

			await member.guild.channels.cache.find(x => x.id === welChan).send(embed)
		}
		else return
	}
